...
ARROW_SEC     = 1           # show arrow before each move executes
MOVE_SEC      = 1           # hold board after each move
FINAL_SEC     = 3           # pause at the very end

TEMP_DIR      = "you_frames"
OUTPUT_VIDEO  = "output_video/you_chess_short.mp4"
...
def send_to_youtube(video_path, title, description, tags=None, privacy="public"):
    """
    Upload a local video file to YouTube as a Short.

    Args:
        video_path  : path to the .mp4 file (e.g. OUTPUT_VIDEO)
    ...
    try:
        ...
        body = {
            "snippet": {
                ...
                "tags":        tags or [],
                "categoryId":  "20",          # Gaming — best for Chess growth
            },
            "status": {
...
yt_title = f"Chess Puzzle Rating {rating} — {side_to_move} to move! #Shorts"
yt_desc  = (
    f"{msg}\n\n"
    f"Puzzle rating: {rating}\n"
    f"{side_to_move} to move — can you find the winning line?\n\n"
    f"{tags}\n\n"
    "#Chess #ChessPuzzles #Shorts"
)
yt_tags = ["chess", "chess puzzle", "tactics", "checkmate", "shorts", "chesssol"]
