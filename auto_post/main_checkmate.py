...
# ── On-board Messages ─────────────────────────────────────
# These are filled in dynamically per puzzle based on verified result.
# See build_puzzle_message() below.
...
# ═══════════════════════════════════════════════════════════
#  CHECKMATE VERIFICATION  ← the core new logic
# ═══════════════════════════════════════════════════════════

def verify_puzzle(fen, moves):
    """
    Play out all moves from the FEN position and return a dict with:

        is_confirmed_mate : bool   — True only if the final position is checkmate
        solver_move_count : int    — number of moves the solver makes (ceil of total)
        mate_label        : str    — e.g. "Mate in 2", or "" if not confirmed mate
        puzzle_type       : str    — "checkmate" | "endgame" | "tactics"
        board_message     : str    — displayed at bottom of board during puzzle
        final_message     : str    — displayed at the very end of the puzzle
        transition_hint   : str    — shown on the transition card for this puzzle

    Always safe to call — catches all exceptions and returns a sane fallback.
    """
    try:
        ...
        n_solver  = math.ceil(len(moves) / 2)   # solver moves = odd-indexed moves
        ...
        if confirmed:
            ...
            # API labelled it CHECKMATE but the sequence doesn't end in mate.
            # Classify it more honestly.
            mate_label      = ""
...
def _classify_non_mate(board, moves):
    """
    Simple heuristic to give a non-mate puzzle a meaningful label.
    Checks the final board state for common tactical/strategic patterns.
    """
    # If it's check but not mate → near-mate combination
    if board.is_check():
        return "combination"

    # Count remaining pieces to guess endgame vs middlegame
...
def _pick_checkmate_message(n_solver):
    """Return a board message appropriate for a verified checkmate puzzle."""
    msgs_by_depth = {
        1: [
...
def _pick_non_mate_message(puzzle_type):
    """Return a board message for a puzzle that does NOT end in checkmate."""
    msgs = {
        "endgame": [
...
def _non_mate_transition(puzzle_type):
    """Short hint shown on the transition card for non-mate puzzles."""
    hints = {
...
def fetch_puzzles(target_count):
    """Fetch checkmate puzzles, de-duplicate by ID."""
    seen_ids = set()
    unique   = []
    max_rounds = 3
...
def create_frame_image(board, last_move=None, arrow_move=None, arrow_color=None,
    ...
    # Mate label (top-right, below counter) — only shown for verified mates
    if mate_label:
...
def create_transition_frame(next_puzzle_num, total_puzzles,
    ...
    """Dark card between puzzles. Shows mate label if verified, or a type hint if not."""
    ...
    # Show verified mate label in red, OR puzzle-type hint in grey
...
def save_puzzle_frames(board, moves, rating, side_to_move,
                       ...
                       verification):
    """
    Generate all frames for one puzzle.
    `verification` is the dict returned by verify_puzzle().
    ...
    common  = dict(
        ...
        mate_label=verification["mate_label"],   # "" for non-mates → not shown
    )
...
def save_transition_frames(next_puzzle_num, total_puzzles, frame_count,
                           next_verification):
    """Transition card using the NEXT puzzle's verified data."""
...
verified_puzzles = []   # list of (board, moves, rating, side_to_move, verification)
...
for idx, (board, moves, rating, side_to_move, verification) in \
    ...
    if idx < total_puzzles:
        _, _, _, _, next_v = verified_puzzles[idx]   # idx is 1-based → next entry
